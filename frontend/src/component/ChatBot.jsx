...
import { IoChatbubbleEllipsesOutline, IoClose, IoSend, IoSparkles } from "react-icons/io5";
    ...
    return (
        <div className="fixed bottom-5 right-5 z-50 flex flex-col items-end gap-4">
            ...
            {isOpen && (
                <div className="bg-white shadow-2xl rounded-2xl border border-gray-100 overflow-hidden flex flex-col w-[90vw] h-[70vh] sm:w-[400px] sm:h-[650px] animate-fade-in origin-bottom-right">
                    ...
                    <div className="bg-gray-900 p-4 flex justify-between items-center text-white shrink-0">
                        ...
                        <button onClick={closeChat} className="p-1 hover:bg-gray-800 rounded-full transition-colors">
                            <IoClose size={20} />
                        </button>
                    </div>
            ...
            {(!isOpen || (isOpen && mode === 'global')) && (
                 <button 
                    onClick={toggleChat}
                    className="w-14 h-14 sm:w-16 sm:h-16 bg-black text-white rounded-full shadow-2xl flex items-center justify-center hover:scale-110 active:scale-90 transition-all duration-300 z-50"
                >
                    {isOpen ? <IoClose size={24} /> : <IoChatbubbleEllipsesOutline size={26} className="sm:w-8 sm:h-8" />}
                </button>
            )}
        </div>
    );
