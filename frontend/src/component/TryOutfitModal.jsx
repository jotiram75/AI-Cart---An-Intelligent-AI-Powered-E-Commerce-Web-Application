...
function TryOutfitModal({
  ...
  const handleGenerate = async () => {
    ...
    try {
      const data = await generateTryOnResult(
        productImageUrl,
        userImage,
        productId,
        userId,
    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
      <div className="bg-white w-full max-w-4xl rounded-3xl overflow-hidden shadow-2xl flex flex-col md:flex-row max-h-[90vh]">
        ...
        <div className="flex-1 bg-gray-50 p-8 flex flex-col relative overflow-y-auto">
          <button
            onClick={onClose}
            className="hidden md:block absolute top-4 right-4 p-2 hover:bg-gray-200 rounded-full transition-colors z-10"
          >
            <IoClose size={24} />
          </button>
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-center space-y-4 text-gray-400">
              ...
              <div>
                ...
                <p className="text-sm max-w-[200px] mx-auto">
                  Upload your photo and click generate to see the magic.
                </p>
              </div>
            </div>
          )}
          ...
          {isGenerating && (
            <div className="flex-1 flex flex-col items-center justify-center space-y-4">
              ...
              <p className="text-sm font-bold text-gray-900 animate-pulse tracking-widest uppercase">
                Processing with VTON...
              </p>
            </div>
          )}
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="bg-white p-3 rounded-2xl border border-gray-100 shadow-sm">
                      ...
                      <p className="text-sm font-bold text-gray-800 capitalize leading-tight">
                        {result.analytics.userAnalysis.bodyShape}
                      </p>
                      <p className="text-xs text-gray-500 capitalize">
                        {result.analytics.userAnalysis.skinTone}
                      </p>
                    </div>
                    <div className="bg-white p-3 rounded-2xl border border-gray-100 shadow-sm">
                      <div className="flex items-center gap-2 mb-2">
                      ...
                      <p className="text-sm font-bold text-gray-800 capitalize leading-tight">
                        {result.analytics.productAnalysis.style}
                      </p>
                      <p className="text-xs text-gray-500 capitalize">
                        {result.analytics.productAnalysis.occasion}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm relative overflow-hidden">
                    ...
                    <div className="flex justify-between items-end mb-2">
                      <div>
                        ...
                        <p className="text-xs text-gray-500 leading-tight pr-4">
                          {result.analytics.compatibility.reason}
                        </p>
                      </div>
                      <span className="text-3xl font-black text-primary leading-none">
                        {result.analytics.compatibility.score}%
                      </span>
                    </div>
                    <div className="w-full bg-gray-100 rounded-full h-1.5 mt-2">
                      <div
                        className="bg-gradient-to-r from-primary to-purple-600 h-1.5 rounded-full transition-all duration-1000"
                        style={{
                          width: `${result.analytics.compatibility.score}%`,
                        }}
                      ></div>
                    </div>
                  </div>
